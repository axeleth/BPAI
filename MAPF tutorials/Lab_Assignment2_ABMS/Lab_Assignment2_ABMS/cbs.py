            ...
            if ac1.speed == ac2.speed: # if the agents have the same speed
                ...
                if ac1.speed > ac2.speed: # if ac1 is faster
                    ...
                    if path1[i-1][1] == path2[j-1][1] + 0.25 and path1[i][1] == path2[j][1]:
                        ...
                        if path2[j-1][0] == path2[j][0]:  # Agent 2 is waiting and agent 2 is there as well
                            if path1[i-1][0] == path2[j-1][0]:
                                if path2[j][1] >= path1[i-1][1] >= path2[j-1][1]:
                                    # print("VERTEX 2.2.1 (agent 1 is faster than agent 2)")
                                    # raise Exception("VERTEX 2.2.1 (agent 1 is faster than agent 2)")
                                    return {'a1': agent1, 'a2': agent2, 'loc': [path1[i-1][0]], 'timestep1': path1[i-1][1], 'timestep2':path2[j][1], 'type': 'vertex2.2.1'}
                    ...
                    # Edge 2.1 – different speed; reaching opposite nodes at the same time
                    if (path2[j-1][1] == path1[i-1][1]-0.25 and path2[j][1] == path1[i][1] and      # if the agents leave their start nodes at different
                        path1[i][0] == path2[j-1][0] and path2[j][0] == path1[i-1][0]):             # times but reach their next nodes simultaneously
                        # print("EDGE 2.1 (agent 1 is faster than agent 2)")
                        # raise Exception("EDGE 2.1 (agent 1 is faster than agent 2)")
                        return {'a1': agent1, 'a2': agent2, 'loc1': [path2[j][0], path1[i][0]], 'loc2':[path1[i-2][0],path2[j][0]], 
                                'timestep1': path2[j][1], 'timestep2':path2[j][1]-0.25, 'type': 'edge2.1'}

                    # Edge 3.1 – different speed; leaving opposite nodes at the same time
                    if (path1[i-1][1] == path2[j-1][1] and path2[j][1] == path1[i][1]+0.25 and      # if the agents leave their start nodes simultaneously
                        path1[i][0] == path2[j-1][0] and path2[j][0] == path1[i-1][0]):          # but reach their next nodes at different times
                        # raise Exception("EDGE 3.1 (agent 1 is faster than agent 2)")
                        return {'a1': agent1, 'a2': agent2, 'loc': [path2[j][0], path1[i][0]], 'timestep1': path2[j][1], 'timestep2':path1[i][1], 'type': 'edge3.1'}
                ...
                if ac1.speed < ac2.speed: # if ac2 is faster
                    ...
                    if path1[i-1][1] == path2[j-1][1] - 0.25 and path1[i][1] == path2[j][1]: # if time is the same and the last step of agent 1 is 0.25 less than the last step of agent 2
                        ...
                        if path1[i-1][0] == path1[i][0]:  # Agent 2 is waiting and agent 2 is there as well
                            if path2[j-1][0] == path1[i-1][0]:
                                if path1[i][1] >= path2[j-1][1] >= path1[i-1][1]:
                                    # print("VERTEX 2.2.1 (agent 1 is faster than agent 2)")
                                    # raise Exception("VERTEX 2.2.1 (agent 1 is faster than agent 2)")
                                    return {'a1': agent1, 'a2': agent2, 'loc': [path2[j-1][0]], 'timestep1': path2[j-1][1], 'timestep2':path1[i][0], 'type': 'vertex2.2.2'}
                                    # return {'a1': agent1, 'a2': agent2, 'loc': [path1[i-1][0]], 'timestep1': path1[i-1][1], 'timestep2':path2[j][1], 'type': 'vertex2.2.1'}
                    ...
                    if (path1[i-1][1] == path2[j-1][1]-0.25 and path1[i][1] == path2[j][1] and      # if the agents leave their start nodes at different
                        path1[i-1][0] == path2[j][0] and path1[i][0] == path2[j-1][0]):             # times but reach their next nodes simultaneously
                    ...
                    # Edge 3.2 – different speed; leaving opposite nodes at the same time
                    if (path1[i-1][1] == path2[j-1][1] and path1[i][1] == path2[j][1]+0.25 and      # if the agents leave their start nodes simultaneously
                        path1[i-1][0] == path2[j][0] and path1[i][0] == path2[j-1][0]):             # but reach their next nodes at different times
...
def standard_splitting(collision):
    """
        ...
        INPUT:
            - collision = dictionary in the form: {'a1': agent, 'a2':agent, 'loc':location, 'timestep':t, 'type':collision_type}
        RETURNS:
    ...
    if collision_type not in ['edge2.1','edge2.2','edge3.1','edge3.2','vertex2.2.1','vertex2.2.2']:  # Edge collisions of type 2 & 3 have 2 separate time steps.
    ...
    if collision_type == 'vertex1': # Vertex 1
        ...
        return_list.append({'agent': agent1, 'loc': location, 'timestep': timestep1})
        ...
        # raise Exception("VERTEX 2.2.1 collision detected :: constraints generated: {}".format(return_list))
        
    
    elif collision_type == 'vertex2.3.1': # Vertex 2.3.1
        location1 = collision['loc1']; location2 = collision['loc2']
        return_list.append({'agent': agent1, 'loc': location1, 'timestep': timestep-0.25})
        ...
        return_list.append({'agent': agent2, 'loc': location, 'timestep': timestep1})
        return_list.append({'agent': agent1, 'loc': location, 'timestep': timestep2}) 
        # raise Exception("VERTEX 2.2.2 collision detected :: constraints generated: {}".format(return_list))
    
    elif collision_type == 'vertex2.3.2': # Vertex 2.3.2
        location1 = collision['loc1']; location2 = collision['loc2']
        return_list.append({'agent': agent2, 'loc': location1, 'timestep': timestep-0.25}) # 
    ...
    # Edge collisions
        ...
        location1 = collision['loc1']; location2 = collision['loc2']
        timestep1 = collision['timestep1']; timestep2 = collision['timestep2']
        return_list.append({'agent': agent1, 'loc': location1, 'timestep': timestep1}) 
        return_list.append({'agent': agent1, 'loc': location2, 'timestep': timestep2}) 
        # raise Exception("EDGE 2.1 collision detected :: constraints generated: {}".format(return_list))

    elif collision_type == 'edge3.1': # Edge 3.1
        location = collision['loc']
        timestep1 = collision['timestep1']; timestep2 = collision['timestep2']
        return_list.append({'agent': agent1, 'loc': location, 'timestep': timestep2})                       
        return_list.append({'agent': agent2, 'loc': [location[-1], location[0]], 'timestep': timestep1})    
        # raise Exception("EDGE 3.1 collision detected :: constraints generated: {}".format(return_list))

        # Agent 2 is faster than agent 1
    elif collision_type == 'edge2.2': # Edge 2.2
        location1 = collision['loc1']; location2 = collision['loc2']
        timestep1 = collision['timestep1']; timestep2 = collision['timestep2']
        return_list.append({'agent': agent2, 'loc': location1, 'timestep': timestep1}) 
        return_list.append({'agent': agent2, 'loc': location2, 'timestep': timestep2}) 
        ...
        timestep1 = collision['timestep1']; timestep2 = collision['timestep2']
        return_list.append({'agent': agent1, 'loc': location, 'timestep': timestep1})                       
        return_list.append({'agent': agent2, 'loc': [location[-1], location[0]], 'timestep': timestep2})    
        # raise Exception("EDGE 3.2 collision detected :: constraints generated: {}".format(return_list))

    else: raise Exception("Collision type '{}' not recognised".format(collision_type))
